#include <Python.h>
#include "string.h"

typedef unsigned char BYTE;

// CRC algorithm function prototype
BYTE CRC8Fast(BYTE *buf_ptr,BYTE *buf_end);

/*=========================================================================*/
const unsigned char crc8revtab[256] ={

                        };

/*=========================================================================*/

static PyObject *customCrc_calc(PyObject *self, PyObject *args)
{
    const char *temp;
    char command[100];
    BYTE crc;
    char crcStr[3];

    if (!PyArg_ParseTuple(args, "s", &temp))
        return NULL;
    strcpy(command, temp);
    crc = CRC8Fast(command,command + strlen(command) - 1);
    sprintf(crcStr, "%02X", crc);
    //printf("CRC calculated: %s\n", crcStr);
    return Py_BuildValue("s", crcStr);
}

/*
 * Bind Python function names to our C functions
 */
static PyMethodDef customCrc_methods[] = {
	{"calcCrc", customCrc_calc, METH_VARARGS},
	{NULL, NULL}
};


/*
 * Python calls this to let us initialize our module
 */
void initcustomCrc()
{
	(void) Py_InitModule("customCrc", customCrc_methods);
}

BYTE CRC8Fast(BYTE *buf_ptr,BYTE *buf_end) {
	static BYTE crc;  /* prevent corruption with global variables */
	crc = 0;   /* 0 for standard CRC, offset for crypto CRC */
	do
	   {
	   crc = crc8revtab[crc ^ *buf_ptr];
	   }
	while(buf_ptr++ < buf_end);
	return(~crc);   /* complimented CRC used to prevent string of 0's */
}
